...
const GENRE_STYLES: Record<string, GenreStyle> = {
  ...
  student: { gradient: 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)', emoji: '🎓' },
...
// ============================================================================
const SaveButton = ({ eventId, saved, onToggle, size = 'medium', requireAuth = false, onAuthRequired, user }: SaveButtonProps): JSX.Element => {
...
// ============================================================================
// EVENT THUMBNAIL COMPONENT (with placeholder)
// ============================================================================
...
export default function Home() {
  ...
  // ============================================================================
  // FORMAT HELPERS - INSIDE COMPONENT
  // ============================================================================
  ...
  // ============================================================================
  // FILTERED DATA - USING FOR LOOPS FOR TYPESCRIPT
  // ============================================================================
  ...
  const filtered = useMemo(() => {
    ...
    for (let i = 0; i < dateFiltered.length; i++) {
      ...
      // Genre filter
      if (activeGenre) {
        if (!e.genres || !e.genres.toLowerCase().includes(activeGenre.toLowerCase())) {
          includeEvent = false
        }
      }
      ...
      // Free filter
      if (showFreeOnly) {
        if (!isFree(e.price_min, e.price_max)) {
          includeEvent = false
        }
      }
  ...
  // ============================================================================
  useEffect(() => {
    localStorage.setItem('so_saved_events', JSON.stringify(Array.from(savedEventIds)))
  }, [savedEventIds])
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          justifyContent: 'space-between',
          alignItems: 'center',
          background: 'linear-gradient(0deg, rgba(10,10,11,1) 0%, rgba(10,10,11,0.95) 70%, transparent 100%)',
  ...
  useEffect(() => {
    ...
    // Group events by venue location for clustering
    ...
    for (let vi = 0; vi < venueKeys.length; vi++) {
      ...
      const handleMarkerClick = (e: any) => {
        ...
        console.log('🎯 Marker clicked!', { count, venueId: v.id })
  ...
  // ============================================================================
  // CLAIM MODAL COMPONENT - COMPREHENSIVE CLAIM SYSTEM
  // ============================================================================
  function ClaimModal({
  ...
  // ============================================================================
        ...
        {/* Actions - Event Actions Component */}
        ...
        <div style={{ 
          ...
          <button 
            ...
            style={{ 
              minHeight: '48px', 
              minWidth: '90px', 
              background: currentIndex === 0 ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #ab67f7, #c490ff)', 
              border: 'none', 
              borderRadius: '24px', 
              padding: '12px 20px', 
              color: currentIndex === 0 ? '#444' : 'white', 
              ...
              fontWeight: 700, 
              cursor: currentIndex === 0 ? 'default' : 'pointer', 
              boxShadow: currentIndex === 0 ? 'none' : '0 4px 16px rgba(171,103,247,0.5)', 
              transition: 'all 200ms ease', 
              ...noSelectStyle 
            }}
          >
          ...
          <button 
            ...
            style={{ 
              minHeight: '48px', 
              minWidth: '90px', 
              background: currentIndex === filtered.length - 1 ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #ab67f7, #c490ff)', 
              border: 'none', 
              borderRadius: '24px', 
              padding: '12px 20px', 
              color: currentIndex === filtered.length - 1 ? '#444' : 'white', 
              ...
              fontWeight: 700, 
              cursor: currentIndex === filtered.length - 1 ? 'default' : 'pointer', 
              boxShadow: currentIndex === filtered.length - 1 ? 'none' : '0 4px 16px rgba(171,103,247,0.5)', 
              transition: 'all 200ms ease', 
              ...noSelectStyle 
            }}
          >
  ...
  // ============================================================================
  // SAVED EVENTS SYNC TO LOCALSTORAGE
  // ============================================================================
  useEffect(() => {
    const saved = Array.from(savedEventIds)
    localStorage.setItem('so_saved_events', JSON.stringify(saved))
  }, [savedEventIds])
  ...
  // ============================================================================
  // RENDER - DESKTOP/TABLET LAYOUT - COMPREHENSIVE DESKTOP VIEW
  // ============================================================================
  if (deviceType === 'desktop' || deviceType === 'tablet') {
    return (
      <div style={{ height: '100vh', width: '100vw', background: '#0a0a0b', display: 'flex', overflow: 'hidden' }}>
        ...
        {/* ONBOARDING & LOGIN MODALS - DESKTOP */}
  ...
  return (
    ...
    >
      ...
      {/* LIST VIEW SHEET - COMPREHENSIVE EVENT LIST */}
      {viewMode === 'list' && (
        <div
          onClick={(e: React.MouseEvent) => e.stopPropagation()}
          style={{
      ...
      {/* ONBOARDING & LOGIN MODALS - MOBILE */}
      {showOnboarding && <OnboardingModal onComplete={completeOnboarding} />}
...
const getGenres = (g: string | null): string | null => {
  return g ? g.split(',').map((x: string) => x.trim()).slice(0, 2).join(' · ') : null
}

const mapsUrl = (v: Venue): string => {
...
const getDateStr = (d: Date): string => getUKDateString(d)

const isTonight = (s: string): boolean => {
  if (!s) return false
  return isUKToday(s)
}

const isTomorrow = (s: string): boolean => {
  if (!s) return false
  return isUKTomorrow(s)
}

const isWeekend = (s: string): boolean => {
  if (!s) return false
  return isUKWeekend(s)
}

const getDateLabel = (s: string): string => {
  if (!s) return 'TBC'
  return getUKDateLabel(s)
}
...
function ClaimModal({
  ...
  claimForm: { name: string; email: string; role: string; proofUrl: string };
  setClaimForm: (form: { name: string; email: string; role: string; proofUrl: string }) => void;
  claimSubmitting: boolean;
  setClaimSubmitting: (submitting: boolean) => void;
...
// ============================================================================
// MOBILE DETAIL SHEET COMPONENT - COMPREHENSIVE MOBILE INTERACTION
// ============================================================================
function MobileDetailSheet({
  ...
  return (
    ...
    >
      ...
      {/* Rest of expanded detail view content continues with complete brand info, venue, tags, price, actions and navigation... */}
      <div style={{ marginBottom: '100px' }}>
        <EventActions 
          event={{ id: current.id, title: current.title, start_time: current.start_time, event_url: current.event_url, sold_out: current.sold_out, price_min: current.price_min, price_max: current.price_max, venue: current.venue }} 
      <div style={{ marginBottom: '100px' }}>
