...
export default function EventActions({
  ...
  const ticketUrl = event.event_url 
    ? (event.event_url.startsWith('http') ? event.event_url : `https://${event.event_url}`)
    : null
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {ticketUrl && (
          ...
          style={{
            ...
            fontSize: '16px',
            fontWeight: 700,
            color: event.sold_out ? '#666' : 'white',
            textDecoration: 'none',
            cursor: event.sold_out ? 'default' : 'pointer',
          }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        <button onClick={handleSave} style={{ ...btnStyle, background: isSaved ? 'rgba(248,113,113,0.15)' : 'rgba(255,255,255,0.05)', border: isSaved ? '1px solid rgba(248,113,113,0.3)' : '1px solid rgba(255,255,255,0.1)' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill={isSaved ? '#f87171' : 'none'} stroke={isSaved ? '#f87171' : '#999'} strokeWidth="2">
            <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
          </svg>
          <span style={{ fontSize: '12px', fontWeight: 600, color: isSaved ? '#f87171' : '#999' }}>{isSaved ? 'Saved' : 'Save'}</span>
        </button>
        
        <button onClick={handleShare} style={{ ...btnStyle, background: copied ? 'rgba(34,197,94,0.15)' : 'rgba(255,255,255,0.05)', border: copied ? '1px solid rgba(34,197,94,0.3)' : '1px solid rgba(255,255,255,0.1)' }}>
          {copied ? (
        ...
        {mapsUrl && (
          <a href={mapsUrl} target="_blank" rel="noopener noreferrer" onClick={handleDirections} style={btnStyle}>
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#999" strokeWidth="2">
        ...
        <button onClick={onClaim} style={btnStyle}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#999" strokeWidth="2">
